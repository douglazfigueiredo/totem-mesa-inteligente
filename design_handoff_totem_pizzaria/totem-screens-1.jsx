...
const Welcome_A = () => (
  <Tablet>
    ...
    >
      <div className="wf-h1" style={{ fontSize: 56, lineHeight: 1 }}>
        oi, mesa 07!
      </div>
      <Scribble w={180} />
      ...
      <div className="wf-btn primary" style={{ fontSize: 22, padding: '14px 40px' }}>
        começar →
      </div>
      <div className="wf-tiny" style={{ marginTop: 10 }}>
        já com pedido aberto? <u>continuar</u>
      </div>
    </div>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 16, marginTop: 12 }}>
            aqui você pede, acompanha o preparo e chama o garçom.
          </div>
        </div>
        <div className="col">
          <div className="wf-btn accent" style={{ fontSize: 18, justifyContent: 'flex-start' }}>
            🍕 ver cardápio
          </div>
          <div className="wf-btn" style={{ fontSize: 16, justifyContent: 'flex-start' }}>
            🛎 chamar garçom
          </div>
          <div className="wf-btn ghost" style={{ fontSize: 14, justifyContent: 'flex-start' }}>
            📋 ver conta
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ marginTop: 24, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {['🍕 pizza', '🍔 lanche', '🥤 bebida', '🍰 doce', '🤔 surpreenda'].map((t) => (
            <div key={t} className="wf-pill" style={{ fontSize: 16, padding: '8px 14px' }}>
              {t}
            </div>
          ))}
        </div>
        <div className="wf-tiny" style={{ marginTop: 22 }}>
          toque uma vibe pra começar →
        </div>
      </div>
      <CallWaiter style={{ bottom: 16, right: 16 }} />
    </div>
...
const Menu_A = () => (
  <Tablet>
    <div style={{ paddingTop: 22 }}>
      <TabBar
        tabs={['pizzas salgadas', 'pizzas doces', 'lanches', 'bebidas', 'sobremesas']}
        active={0}
      />
    ...
    <div style={{ display: 'flex', height: '100%', paddingTop: 22 }}>
      ...
      <div style={{ flex: 1, padding: 14, overflow: 'hidden' }}>
        <div className="wf-h2" style={{ marginBottom: 10 }}>
          pizzas <span style={{ color: 'var(--accent)' }}>·</span> 18 opções
        </div>
        <div className="col" style={{ gap: 8 }}>
          {[
            'margherita — molho, mussarela, manjericão',
            'calabresa — calabresa fininha, cebola',
            'portuguesa — presunto, ovo, ervilha',
            'quatro queijos — mussarela, gorgon., parm., catup.',
          ].map((d, i) => (
            <div key={i} className="row wf-box" style={{ padding: 8, gap: 10 }}>
              <div className="wf-photo" style={{ width: 60, height: 50 }}>
                foto
              </div>
              ...
              <div className="wf-btn" style={{ fontSize: 14, padding: '4px 10px' }}>
                +
              </div>
            </div>
          ))}
...
const Menu_C = () => (
  <Tablet>
    ...
    <div style={{ paddingTop: 28, height: '100%' }}>
      ...
      <div
        style={{ padding: 14, display: 'flex', gap: 10, alignItems: 'stretch', overflow: 'hidden' }}
      >
        {[0, 1, 2].map((i) => (
          <div
            key={i}
          ...
          >
            <div className="wf-photo" style={{ height: i === 0 ? 200 : 130 }}>
              foto grande
            </div>
            <div className="wf-h3">pizza calabresa {i === 0 && '⭐'}</div>
            <div className="wf-tiny">
              calabresa fatiada, cebola roxa, azeitona preta, manjericão fresquinho
            </div>
            <div className="row between">
              <span className="wf-mono">R$ 64</span>
              <span className="wf-btn accent" style={{ fontSize: 14, padding: '4px 12px' }}>
                +
              </span>
            </div>
          </div>
  ...
  <Tablet>
    <div style={{ paddingTop: 22, display: 'flex', height: '100%' }}>
      <div style={{ flex: 1, padding: 16 }}>
        <div className="wf-photo" style={{ height: 240, marginBottom: 12 }}>
          foto · pizza calabresa 30cm
        </div>
        <div className="wf-h2">pizza calabresa</div>
        <div className="wf-tiny" style={{ marginTop: 4 }}>
          calabresa fatiada, cebola roxa, azeitona preta, orégano
        </div>
      </div>
      <div style={{ flex: 1, padding: 16, borderLeft: '1.5px dashed var(--line)' }}>
        ...
        <div className="row" style={{ marginBottom: 14 }}>
          {['broto', 'média', 'grande'].map((s, i) => (
            <div key={s} className={`wf-pill${i === 2 ? ' active' : ''}`} style={{ fontSize: 13 }}>
              {s}
            </div>
          ))}
        </div>
        ...
        <div className="row" style={{ marginBottom: 14, flexWrap: 'wrap' }}>
          {['tradicional', 'catupiry +6', 'cheddar +6'].map((s, i) => (
            <div key={s} className={`wf-pill${i === 0 ? ' active' : ''}`} style={{ fontSize: 13 }}>
              {s}
            </div>
          ))}
        </div>
        ...
        <div className="row between" style={{ marginTop: 14 }}>
          <div className="row">
            <div className="wf-btn" style={{ width: 32, padding: 0 }}>
              −
            </div>
    ...
    >
      ...
      <div className="row" style={{ gap: 16, alignItems: 'flex-start' }}>
        ...
        <div className="grow col" style={{ gap: 8 }}>
          ...
          <div className="wf-tiny" style={{ marginTop: 4 }}>
            preço pelo sabor mais caro · R$ 78
          </div>
        </div>
      </div>
        ...
        <div className="col" style={{ width: 200, gap: 6 }}>
          ...
          {[
            ...
            <div
              key={t}
              className="row between wf-box"
              style={{ padding: '4px 8px', fontSize: 12, cursor: 'grab' }}
            >
              <span>≡ {t}</span>
              <span className="wf-mono" style={{ color: 'var(--accent)' }}>
                +R${i + 1}
              </span>
            </div>
          ))}
      ...
      <div className="row" style={{ gap: 12, alignItems: 'stretch' }}>
        <div className="wf-box" style={{ flex: 1, padding: 10, borderColor: 'var(--accent)' }}>
          <div className="wf-mono row between" style={{ color: 'var(--accent)' }}>
            <span>🍕 PRA COZINHA</span>
            <span>~22 min</span>
          </div>
          <div className="col" style={{ gap: 6, marginTop: 8, fontSize: 13 }}>
            <div className="row between">
              <span>1× pizza calabresa G</span>
              <span className="wf-mono">R$ 78</span>
            </div>
            <div className="row between">
              <span>2× hamb. cheddar</span>
              <span className="wf-mono">R$ 84</span>
            </div>
          </div>
        </div>
        <div className="wf-box" style={{ flex: 1, padding: 10, borderColor: 'var(--ink-2)' }}>
          <div className="wf-mono row between">
            <span>🛎 PRO GARÇOM</span>
            <span>~5 min</span>
          </div>
          <div className="col" style={{ gap: 6, marginTop: 8, fontSize: 13 }}>
            <div className="row between">
              <span>2× guaraná 600ml</span>
              <span className="wf-mono">R$ 18</span>
            </div>
            <div className="row between">
              <span>1× brownie</span>
              <span className="wf-mono">R$ 16</span>
            </div>
          </div>
        </div>
      </div>
...
const Cart_C = () => (
  <Tablet>
    ...
    <div style={{ paddingTop: 22, padding: 16, height: '100%' }}>
      <div className="wf-h2" style={{ marginBottom: 8 }}>
        como vai chegar
      </div>
      <div className="wf-tiny" style={{ marginBottom: 14 }}>
        itens chegam em ondas conforme ficam prontos
      </div>
      <div className="col" style={{ gap: 0, position: 'relative' }}>
        {[
          ...
          {
            t: '~ 12 min',
            items: ['hamb. cheddar bacon ×2', 'fritas'],
            color: 'var(--accent)',
            icon: '🍔',
          },
          ...
          >
            ...
            <div className="grow">
              <div className="wf-mono" style={{ color: w.color }}>
                {w.t}
              </div>
  ...
  <Tablet>
    <div style={{ paddingTop: 22, height: '100%', padding: 20, display: 'flex', gap: 20 }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div className="wf-h1" style={{ fontSize: 40, marginTop: 6 }}>
          foi! ✨
        </div>
        <div style={{ marginTop: 12, fontSize: 14 }}>
          cozinha recebeu, vai começar quando estiver na fila.
        </div>
        <div className="col" style={{ marginTop: 18, gap: 6 }}>
          <div className="row" style={{ gap: 8 }}>
            <span className="dot waiting" />
            <span style={{ fontSize: 13 }}>recebido — agora</span>
          </div>
          <div className="row" style={{ gap: 8 }}>
            <span className="dot waiting" />
            <span style={{ fontSize: 13, color: 'var(--ink-3)' }}>em preparo — em breve</span>
          </div>
          <div className="row" style={{ gap: 8 }}>
            <span className="dot waiting" />
            <span style={{ fontSize: 13, color: 'var(--ink-3)' }}>pronto — depois</span>
          </div>
        </div>
      </div>
      <div
      ...
      >
        ...
        <div style={{ borderTop: '1px dashed var(--line)', marginTop: 10, paddingTop: 8 }}>
          <div className="row between">
            <span>total</span>
            <span className="wf-mono">R$ 196</span>
          </div>
        </div>
        <div className="wf-btn ghost" style={{ marginTop: 10, width: '100%', fontSize: 13 }}>
          ver detalhes
        </div>
      </div>
    </div>
  ...
  <Tablet>
    ...
    >
      ...
      >
        <div
          className="row between"
          style={{ borderBottom: '2px dashed var(--line)', paddingBottom: 8 }}
        >
        ...
        <div
          className="row between"
          style={{ marginTop: 14, borderTop: '2px dashed var(--line)', paddingTop: 10 }}
        >
          <div className="wf-btn ghost" style={{ fontSize: 13 }}>
            + pedir mais
          </div>
          <div className="wf-btn primary" style={{ fontSize: 14 }}>
            acompanhar →
          </div>
        </div>
      </div>
