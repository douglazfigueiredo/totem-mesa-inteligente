...
import type { Order, Table, WaiterCall, WSEvent } from '@app/schemas';
...
export const useSocketLifecycle = () => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('state:sync', {}, (snap: unknown) => {
        if (snap && typeof snap === 'object') {
          const s = snap as {
            activeOrders?: Order[];
            pendingWaiterCalls?: WaiterCall[];
            tables?: Table[];
          };
          useWaiterStore.getState().setSnapshot({
            calls: s.pendingWaiterCalls ?? [],
    ...
    socket.on('event', (raw: unknown) => {
      try {
        useWaiterStore.getState().applyEvent(raw as WSEvent);
      } catch {
        // silent
      }
    });
