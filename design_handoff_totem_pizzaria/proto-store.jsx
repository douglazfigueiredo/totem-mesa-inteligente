...
function storeReducer(s, a) {
  switch (a.type) {
    case 'goto':
      return { ...s, view: a.view, detailItem: a.item ?? s.detailItem };
    case 'addToCart': {
    ...
    case 'incCart':
      return { ...s, cart: s.cart.map((c) => (c.id === a.id ? { ...c, qty: c.qty + 1 } : c)) };
    case 'decCart':
      return {
        ...s,
        cart: s.cart
          .map((c) => (c.id === a.id ? { ...c, qty: Math.max(0, c.qty - 1) } : c))
          .filter((c) => c.qty > 0),
      };
    case 'removeCart':
    ...
    case 'sendOrder': {
      ...
      const ts = Date.now();
      if (cozinha.length)
        orders.push({ id: 'k' + ts, items: cozinha, status: 'queue', ts, kind: 'cozinha' });
      if (garcom.length)
        orders.push({ id: 'g' + ts, items: garcom, status: 'queue', ts, kind: 'garcom' });
    ...
    case 'startPrep': {
      ...
      const dur =
        a.duration ??
        Math.max(...s.orders.find((o) => o.id === a.id).items.map((c) => c.item.tempo));
    ...
    case 'ackCall':
      return { ...s, calls: s.calls.map((c) => (c.id === a.id ? { ...c, status: 'ack' } : c)) };
    case 'minimize':
      return { ...s, miniTimer: a.value };
    case 'toggleTax':
      return { ...s, taxOn: !s.taxOn };
    case 'reset':
      return initialState;
    default:
      return s;
