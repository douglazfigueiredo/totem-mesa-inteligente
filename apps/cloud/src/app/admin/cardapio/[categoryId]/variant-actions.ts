...
export async function createVariantAction(
  ...
  return withFeedback('variante criada', async () => {
...
export async function updateVariantAction(
  ...
  return withFeedback('variante atualizada', async () => {
    const productId = IdSchema.parse(formData.get('productId'));
    const product = await ensureProductInTenant(productId);
...
export async function toggleVariantAvailableAction(
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
  return withFeedback('', async () => {
...
export async function deleteVariantAction(
  ...
  return withFeedback('variante removida', async () => {
    const productId = IdSchema.parse(formData.get('productId'));
    const product = await ensureProductInTenant(productId);
    const id = IdSchema.parse(formData.get('id'));
