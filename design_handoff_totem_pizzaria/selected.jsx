...
const Sel_Detail = () => (
  <Tablet>
    <div style={{ paddingTop: 22, height: '100%', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div className="row" style={{ gap: 14, alignItems: 'flex-start' }}>
        ...
        <div className="grow col" style={{ gap: 6 }}>
          ...
          {['calabresa','margherita','portuguesa'].map(s => (
            <div key={s} className="row between wf-box" style={{ padding: '5px 10px', fontSize: 12 }}>
              <span>· {s}</span>
              <span className="wf-tiny" style={{ color: 'var(--alert)', cursor: 'pointer' }}>remover ×</span>
            </div>
          ))}
      ...
      <div className="row" style={{ gap: 14, alignItems: 'flex-start', marginTop: 4 }}>
        <div style={{ flex: 1 }}>
          <div className="wf-mono" style={{ marginBottom: 4 }}>BORDA</div>
          <div className="row" style={{ flexWrap: 'wrap', gap: 6 }}>
            {['tradicional','catupiry +6','cheddar +6'].map((s,i) => (
              <div key={s} className={`wf-pill${i===1?' active':''}`} style={{ fontSize: 11, padding: '3px 8px' }}>{s}</div>
            ))}
          </div>
        </div>
        <div style={{ flex: 1.2 }}>
          <div className="wf-mono" style={{ marginBottom: 4 }}>ADICIONAIS</div>
          <div className="row" style={{ flexWrap: 'wrap', gap: 4, fontSize: 11 }}>
            {[['+ bacon','+R$5',true],['+ catupiry','+R$4',false],['+ azeitona','+R$2',false],['+ cebola','+R$2',true]].map(([n,p,on]) => (
              <div key={n} className={`wf-pill${on?' accent':''}`} style={{ fontSize: 11, padding: '3px 8px' }}>
                {on ? '✓' : '+'} {n}
              </div>
            ))}
          </div>
      ...
      <div className="row between" style={{ marginTop: 'auto' }}>
        ...
        <div className="row" style={{ gap: 6 }}>
          <div className="wf-btn" style={{ width: 32, padding: 0, fontSize: 14 }}>−</div>
          <span className="wf-h3" style={{ minWidth: 18, textAlign: 'center' }}>1</span>
          <div className="wf-btn" style={{ width: 32, padding: 0, fontSize: 14 }}>+</div>
          <div className="wf-btn primary" style={{ marginLeft: 8 }}>add R$ 91</div>
        </div>
      </div>
    </div>
  </Tablet>
          ...
          <div key={i} className="wf-box row" style={{ padding: 10, gap: 10 }}>
            ...
            <div className="grow">
              ...
              <div className="row" style={{ gap: 8, marginTop: 4 }}>
                <span className="wf-tiny" style={{ color: 'var(--accent)', cursor: 'pointer' }}>✏ editar</span>
                <span className="wf-tiny" style={{ color: 'var(--alert)', cursor: 'pointer' }}>× remover</span>
              </div>
            </div>
            <div className="row" style={{ gap: 6 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        ...
        <div className="row" style={{ gap: 8, marginTop: 4 }}>
          {[
            ...
            { n: 'margherita M', pct: 0.30, time: '12:10' },
            { n: 'portuguesa G', pct: 0.85, time: '02:40' },
          ].map((p,i) => (
            <div key={i} className="wf-box" style={{
              padding: 6, textAlign: 'center', minWidth: 70,
              ...
              <div className="wf-tiny" style={{ marginTop: 2 }}>{p.n}</div>
              <div className="wf-mono" style={{ fontSize: 10 }}>{p.time}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8, justifyContent: 'center' }}>
        ...
        <div className="col" style={{ gap: 5, fontSize: 12 }}>
          <div className="row between wf-box" style={{ padding: 7 }}>
            ...
            <span className="wf-pill accent" style={{ fontSize: 10, padding: '1px 7px' }}>preparando</span>
          </div>
          <div className="row between wf-box" style={{ padding: 7 }}>
          ...
          <div className="row between wf-box" style={{ padding: 7 }}>
            ...
            <span className="wf-pill accent" style={{ fontSize: 10, padding: '1px 7px' }}>quase</span>
          </div>
          <div className="row between wf-box" style={{ padding: 7 }}>
            <span>· hamb. cheddar ×2</span>
            <span className="wf-pill" style={{ fontSize: 10, padding: '1px 7px' }}>na fila</span>
          </div>
          <div className="row between wf-box" style={{ padding: 7 }}>
            <span>· guaraná ×2</span>
            <span className="wf-pill ready" style={{ fontSize: 10, padding: '1px 7px' }}>servido ✓</span>
          </div>
        </div>
        <div className="wf-btn ghost" style={{ marginTop: 8, fontSize: 13 }}>+ pedir mais (sem perder timer)</div>
      </div>
    </div>
...
const Sel_Ready = () => (
  <Tablet>
    ...
    {/* Dim layer */}
    <div style={{
      position: 'absolute', inset: 22, background: 'rgba(26,23,20,0.45)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      {/* Card overlay */}
      <div className="wf-box wobble" style={{
        width: 460, padding: 24, background: 'var(--ready-soft)',
        ...
        <div className="wf-h1" style={{ fontSize: 44 }}>tá pronto!</div>
        <Scribble w={140} color="var(--ready)" />
        <div style={{ fontSize: 15, textAlign: 'center', maxWidth: 340 }}>
          a pizza calabresa saiu do forno.<br/>
          o garçom já tá a caminho da mesa.
        </div>
        <div className="wf-tiny">fecha em 8s · você volta pro cardápio</div>
        <div className="row" style={{ marginTop: 6, gap: 8 }}>
          <div className="wf-btn ghost" style={{ fontSize: 13 }}>silenciar 🔕</div>
          <div className="wf-btn ready">ok, voltar →</div>
        </div>
      </div>
    </div>
    <Annotate style={{ top: 50, left: 18, color: 'var(--paper)' }}>↑ overlay sobre onde estava · ok = volta</Annotate>
...
const Sel_Waiter = () => (
  <Tablet>
    <div style={{ paddingTop: 22, height: '100%', padding: 24, position: 'relative',
      ...
      <div className="wf-box" style={{
        padding: '10px 16px', marginTop: 6, background: 'var(--accent-soft)',
        ...
        <div className="wf-btn ghost" style={{ fontSize: 11, padding: '3px 8px' }}>cancelar</div>
      </div>
      <div className="wf-tiny" style={{ marginTop: 4 }}>swipe-to-call evita chamado por toque acidental</div>
    </div>
  </Tablet>
);
  ...
  <Tablet>
    <div style={{ paddingTop: 22, padding: 16 }}>
      ...
      <div className="wf-box fill" style={{ padding: 10, marginTop: 14 }}>
        <div className="row between"><span>subtotal</span><span className="wf-mono">R$ 227,00</span></div>
        <div className="row between" style={{ marginTop: 4 }}>
          <span className="row" style={{ gap: 8 }}>
            <span className="wf-pill" style={{
              padding: '0 6px', background: 'var(--line)', color: 'var(--paper)',
              border: '1.5px solid var(--line)', fontSize: 11, minWidth: 18, justifyContent: 'center',
            }}>✓</span>
            serviço 10% (opcional)
          </span>
          <span className="wf-mono">R$ 22,70</span>
        </div>
        <div className="row between" style={{ marginTop: 6, fontSize: 18, fontFamily: 'var(--display)' }}>
          <span>total</span><span>R$ 249,70</span>
        </div>
        <div className="wf-tiny" style={{ marginTop: 2 }}>desmarque pra remover a taxa</div>
      </div>
...
Object.assign(window, {
  Sel_Welcome, Sel_Menu, Sel_Detail, Sel_Cart, Sel_Confirm,
  Sel_Timer, Sel_Mini, Sel_Ready, Sel_Waiter, Sel_History,
  Sel_KDS, Sel_Garcom,
});
