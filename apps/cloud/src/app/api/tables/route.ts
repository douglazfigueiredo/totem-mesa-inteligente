...
import { TablesSnapshot } from '@app/schemas';
...
export async function GET(req: Request) {
  ...
  const [rows, [{ latest }]] = await Promise.all([
    db
      .select({
        id: schema.tables.id,
        numero: schema.tables.numero,
        capacidade: schema.tables.capacidade,
        isActive: schema.tables.isActive,
      })
      .from(schema.tables)
      .where(eq(schema.tables.tenantId, hub.tenantId))
      .orderBy(schema.tables.numero),
    db
      .select({ latest: max(schema.tables.updatedAt) })
      .from(schema.tables)
      .where(eq(schema.tables.tenantId, hub.tenantId)),
  ]);
  ...
  const parsed = TablesSnapshot.safeParse(snapshot);
  if (!parsed.success) {
    console.error('[tables] validação falhou', parsed.error.issues);
  ...
  await lastSeenPromise.catch((err) => {
    console.warn('[tables] falha ao atualizar last_seen_at:', err);
  });

  return NextResponse.json(parsed.data, {
    headers: {
      'Cache-Control': 'no-store',
      'X-Tables-Version': String(updatedAt),
    },
  });
}
