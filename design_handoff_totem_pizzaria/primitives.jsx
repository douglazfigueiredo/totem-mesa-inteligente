...
const Scribble = ({ w = 80, color = 'var(--accent)' }) => (
  <svg width={w} height="6" viewBox={`0 0 ${w} 6`} style={{ display: 'block' }}>
    <path
      d={`M 2 4 Q ${w * 0.25} 1, ${w * 0.5} 3 T ${w - 2} 3`}
      fill="none"
      stroke={color}
      strokeWidth="2"
      strokeLinecap="round"
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center' }}>
        <div className="wf-h1" style={{ fontSize: size * 0.32 }}>
          {label}
        </div>
        {sub && (
          <div className="wf-mono" style={{ marginTop: 2 }}>
            {sub}
          </div>
        )}
      </div>
...
const PizzaBake = ({ size = 80, pct = 0.6 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" style={{ display: 'block' }}>
    ...
    {/* heat lines */}
    <path
      d="M 30 50 q 2 -3 0 -6 M 50 50 q 2 -3 0 -6 M 70 50 q 2 -3 0 -6"
      fill="none"
      stroke="var(--accent)"
      strokeWidth="1.5"
      strokeLinecap="round"
...
const TabBar = ({ tabs, active = 0 }) => (
  ...
  >
    {tabs.map((t, i) => (
      <div
        key={i}
        className={`wf-pill${i === active ? ' active' : ''}`}
...
const MenuCard = ({ name, price, w = 140, h = 90 }) => (
  ...
  >
    <div className="wf-photo" style={{ flex: 1 }}>
      foto
    </div>
    <div
      style={{
...
const CallWaiter = ({ style = {}, label = 'chamar garçom 🛎' }) => (
  <div className="wf-btn" style={{ position: 'absolute', ...style, fontSize: 13 }}>
    {label}
  </div>
);
